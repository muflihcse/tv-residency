  ...
  const filteredVillas = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(v => 
        ...
        v.view.toLowerCase().includes(q) ||
        v.location.toLowerCase().includes(q)
      );
    }

    // Max Price
    result = result.filter(v => v.priceINR <= maxPrice);

    // Bedrooms
    if (selectedBedrooms > 0) {
      result = result.filter(v => v.bedrooms === selectedBedrooms);
    }
    ...
    return result;
  }, [villasList, searchQuery, maxPrice, selectedBedrooms, minRating, sortBy]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          ...
          <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl font-bold text-primary dark:text-white tracking-tight">
            Private Luxury Pool Villas
          </h1>
          <div className="w-16 h-0.5 bg-warm-gold mx-auto mt-3 mb-4"></div>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
            Exclusive 2 to 4-bedroom secluded residences featuring private heated infinity pools, 24/7 dedicated butlers, and in-villa chef kitchens.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-[#15171C] p-4 rounded-2xl shadow-level-2 border border-gray-100 dark:border-white/10 mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="relative w-full md:w-96">
            ...
            <input
              type="text"
              placeholder="Search villas by name, pool, or ocean view..."
              value={searchQuery}
        ...
        {/* 2-Column Grid: Sidebar Filters + Villa Cards */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <aside className={`lg:col-span-3 bg-white dark:bg-[#15171C] rounded-2xl p-6 border border-gray-100 dark:border-white/10 shadow-level-2 space-y-6 ${
            ...
            <div className="flex justify-between items-center pb-4 border-b border-gray-100 dark:border-white/10">
              <span className="font-serif font-bold text-sm text-primary dark:text-white">Filter Villas</span>
            ...
            <div>
              ...
              <div className="space-y-1">
                {[
                  ...
                  { val: 4.95, label: '★ 4.95+ Signature Collection' },
                ].map((r) => (
                  <button
                    key={r.val}
            ...
            {/* All Villas Include Pill */}
            <div className="p-4 rounded-xl bg-warm-gold/10 border border-warm-gold/30 text-xs text-gray-700 dark:text-gray-300 space-y-1.5">
              <div className="font-bold text-warm-gold uppercase tracking-wider text-[10px]">
                Included in All Villas:
              </div>
              <div>• Free Covered / Dedicated Parking</div>
              <div>• High-Speed Wi-Fi (All Rooms)</div>
              <div>• Water Heater / Geyser in All Baths</div>
              <div>• Climate Control AC & Smart 4K TV</div>
              <div>• 24/7 Room Service & Power Backup</div>
          ...
          <div className="lg:col-span-9 space-y-6">
            <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400">
              <span>Showing <strong className="text-primary dark:text-white">{filteredVillas.length}</strong> private villas</span>
            </div>
            ...
            {filteredVillas.length === 0 ? (
              <div className="text-center py-20 bg-white dark:bg-[#15171C] rounded-2xl border border-gray-100 dark:border-white/10 p-8">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 max-w-sm mx-auto mb-5">
                  Try adjusting your bedroom requirements or price range.
                </p>
                <button
                  onClick={handleReset}
